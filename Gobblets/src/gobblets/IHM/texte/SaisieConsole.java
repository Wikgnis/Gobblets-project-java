...
public class SaisieConsole extends IHM {
    ...
    private Menu displayMenuLangue() {
        String[] textMenu = {"Fran�ais", "English", "Deutsche", getLanguage().menu(Menu.MENU_QUITTER)};
        for (int i = 0; i < textMenu.length; i++) {
            System.out.print(i + 1 + " - " + textMenu[i]);
            if (i == 0 && getLanguage() instanceof Francais) System.out.print(" <Actuellement utilis�e>");
